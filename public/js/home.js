...
$(function () {
  ...
  $('#search').keypress(function (event) {
    if (event.keyCode === 13) {
      queryAndPage($(this).val());
    }
  });

  $('#search').next('button').on('click', function () {
    queryAndPage($('#search').val());
  });
  ...
  function initPagination(currentPage, totalPages) {
    $('#pagination').remove();
    $('.page-foot').html('<div id="pagination" totalPages=' + totalPages + '></div>');
    $('#pagination').twbsPagination({
      ...
      onPageClick: function (event, page) {
        $.get('/page-content', {page: page, query: $('#search').val()}, function (result) {
          $('#page-content').html(result);
